...
if __name__ == '__main__':
    ...
    mobile_variant = bot_api.create_variant(main_message.id, 'Телефоны')
    computer_variant = bot_api.create_variant(main_message.id, 'Компьютеры')
    appliances_variant = bot_api.create_variant(main_message.id, 'Бытовая техника')
    ...
    android_variant = bot_api.create_variant(mobile_message.id, 'Android')
    iphone_variant = bot_api.create_variant(mobile_message.id, 'IPhone')
    mobile_cancel_variant = bot_api.create_variant(mobile_message.id, 'Вернуться в начало')
