...
if __name__ == '__main__':
    ...
    mobile_variant_id = bot_api.create_variant(main_message_id, 'Телефоны')
    computer_variant_id = bot_api.create_variant(main_message_id, 'Компьютеры')
    appliances_variant_id = bot_api.create_variant(main_message_id, 'Бытовая техника')

    mobile_message_id = bot_api.create_message(bot_id, 'Какие телефоны предпочитаете?', 100, 130)
    bot_api.connect_variant(mobile_variant_id, mobile_message_id)

    computer_message_id = bot_api.create_message(bot_id, 'Какие компьютеры предпочитаете?', 200, 150)
    bot_api.connect_variant(computer_variant_id, computer_message_id)

    android_variant_id = bot_api.create_variant(mobile_message_id, 'Android')
    iphone_variant_id = bot_api.create_variant(mobile_message_id, 'IPhone')
    mobile_cancel_variant_id = bot_api.create_variant(mobile_message_id, 'Вернуться в начало')

    android_select_message_id = bot_api.create_message(
        bot_id, 'Выберите телефон который хотите приобрести', 200, 250)
    bot_api.connect_variant(android_variant_id, android_select_message_id)
    ...
    bot_api.connect_variant(iphone_variant_id, iphone_select_message)
    ...
    bot_api.connect_variant(mobile_cancel_variant_id, main_message_id)
