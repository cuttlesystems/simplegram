...
from b_logic.data_objects import BotMessage, ButtonTypes
...
class MessageEditorDialog(QDialog):
    ...
    def set_message(self, message: BotMessage):
        ...
        self._ui.radio_reply.setChecked(message.keyboard_type == ButtonTypes.REPLY)
        self._ui.radio_inline.setChecked(message.keyboard_type == ButtonTypes.INLINE)

    def message_text(self) -> str:
        return self._ui.message_text_edit.toPlainText()

    def keyboard_type(self) -> Optional[ButtonTypes]:
        button_types: Optional[ButtonTypes] = None
        if self._ui.radio_reply.isChecked():
            button_types = ButtonTypes.REPLY
        elif self._ui.radio_inline.isChecked():
            button_types = ButtonTypes.INLINE
        return button_types
