...
class SelectedProjectWidget(QWidget):
    ...
    def _switchBot(self):
        if(self._ui.switch_activated_bot.isChecked()):
            self._ui.marker_activision_bot.setStyleSheet("QLabel{border-radius:8px; border:none; color:white;"
                                                       "background-color:#4DAAFF;}")
            self._ui.marker_activision_bot.setText(self._tr(u"Bot is enabled"))
            self.activated_bot_signal.emit(True)
        else:
            self._ui.marker_activision_bot.setStyleSheet("QLabel{border-radius:8px; border:none; color:white;"
                                                       "background-color:#FF5F8F;}")
            self._ui.marker_activision_bot.setText(self._tr(u"Bot is disabled"))
